...
- Reader deep links are restored from `td-chap` and `td-page` URL state during auto-open.
...
- Reader state is encoded in URL query params, primarily `td-chap` and `td-page`, with hash parsing kept for older links.
